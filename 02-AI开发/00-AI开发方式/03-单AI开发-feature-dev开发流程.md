...
**核心思路**：测试 → 失败 → 分析 → 修复 → 再测试，直到通过。

**在测试阶段使用提示词驱动**：

```
# 场景1：自动修复测试失败
"运行所有测试，分析失败原因，逐个修复，修复后重新测试，直到全部通过"

# 场景2：补充测试覆盖率
"为 src/auth.ts 添加单元测试，目标覆盖率 > 80%，运行后修复失败的用例"

# 场景3：修复 linter 错误
"运行 ESLint 检查，修复所有错误后重新检查，直到 0 错误"
```
...
**注意事项：**
- 明确完成标准（如"所有测试通过"），避免无限循环
- 适合 Bug 修复、补充测试，不适合需要人工决策的任务
- 如果使用 Anthropic 原生 API，可安装 [Ralph Wiggum 插件](./06-工具-Ralph%20Wiggum自动迭代插件.md) 获得基于 Stop Hook 的自动迭代体验
...
### 方式一：提示词直接驱动（推荐，适用于 GLM API 等第三方接口）

> **无需安装任何插件**，直接在 Claude Code 对话中粘贴提示词即可启动完整工作流。

本 SOP 的核心是**方法论**而非具体插件。Claude Code 内置的 Task 子代理已具备多代理并行能力，直接用自然语言驱动即可。

**快速启动**：复制 [一键执行完整流程](#-一键执行完整流程8阶段完整版) 中的提示词，粘贴到 Claude Code 对话中。

**分阶段启动**：

| 阶段 | 提示词 |
|------|--------|
| 阶段一：反向面试 | `"开始反向面试：向我提问确认以下需求 [描述你的功能]，生成需求文档"` |
| 阶段二：探索代码库 | `"探索当前代码库的架构、规范和可复用资源，生成资源清单"` |
| 阶段三：设计方案 | `"基于以上需求和代码库，设计技术方案（不写代码），生成架构文档"` |
| 阶段四：实施开发 | `"基于设计方案，开始编写代码"` |
| 阶段五：测试 | `"为刚才实现的代码编写测试用例并执行，生成测试报告"` |
| 阶段六：审查 | `"审查刚才的代码变更，从安全、性能、逻辑三个维度生成审查报告"` |
| 阶段七：发布 | `"执行发布前检查，确认所有测试通过后提交代码"` |
| 阶段八：复盘 | `"帮我复盘这次开发过程，分析技术决策、问题和改进建议"` |

**优势**：
- 不依赖特定插件，任何 AI 编码工具都适用
- 不消耗额外 token（插件模式会启动多个子代理，token 消耗翻倍）
- 灵活调整：可根据任务复杂度跳过或合并阶段

### 方式二：安装 feature-dev 插件（适用于 Anthropic 原生 API）

> 如果你使用的是 Anthropic 官方 API（非第三方中转），可以安装官方插件获得更自动化的体验。
...
安装后使用 `/feature-dev` 命令即可启动 7 阶段流水线。

**注意**：插件的多代理编排会指定特定模型（如 `model: sonnet`），使用第三方 API 时可能出现兼容性问题。
...
【阶段一：反向面试】
向我提问并确认需求，生成需求文档
功能：用户认证系统，支持邮箱/密码登录、JWT Token 鉴权、密码加密存储、防暴力破解
...
【阶段一：反向面试】
向我提问并确认需求，生成需求文档
功能：API 性能优化，目标响应时间 < 200ms（当前 800ms）

【阶段二：探索代码库】
探索现有代码（API端点、数据库查询、缓存现状）

【阶段三：设计方案】
设计优化方案（数据库索引、Redis缓存、CDN配置）
...
【阶段一：反向面试】
向我提问并确认需求，生成需求文档
功能：重构数据访问层，引入 Repository 模式，支持多种数据库

【阶段二：探索代码库】
探索现有代码（数据访问层、数据库配置）

【阶段三：设计方案】
设计 Repository 接口（BaseRepository、CRUD操作）
...
### v2.1 (2026-04-25)
- [OK] 重写：安装指南改为"使用指南"，新增提示词直接驱动方式（适用于 GLM API 等第三方接口）
- [OK] 更新：Ralph Wiggum 插件引用改为提示词驱动的迭代修复循环
- [OK] 更新：实战案例中的 `/feature-dev` 命令替换为完整提示词模板
