# Let's Extract Contact Extractor — 搜索任务清单

## 使用方法
把同一组的「关键词」和「城市」分别批量粘贴到工具的对应输入框，一次跑完该组所有组合。

---

## P0 — 汽车压铸/铸造

### P0-A：压铸厂（核心搜索）

**关键词（全部粘贴）：**
```
die casting company
aluminum die casting
automotive casting supplier
die casting manufacturer
aluminium die casting
```

**城市（全部粘贴）：**
```
Monterrey
Saltillo
Queretaro
Ramos Arizpe
Aguascalientes
Puebla
Detroit
Toledo
Cleveland
Grand Rapids
Huntsville
Indianapolis
Chattanooga
Spartanburg
Stuttgart
Nürnberg
Hannover
Munich
Ingolstadt
Wolfsburg
Bursa
Kocaeli
Izmit
Nagoya
Toyota
Rayong
Chonburi
Pune
Chennai
Gurugram
```

= 5 关键词 × 30 城市 = **150 条组合**

---

### P0-B：汽车具体零部件铸造厂

**关键词（全部粘贴）：**
```
brake caliper manufacturer
brake disc manufacturer
steering knuckle manufacturer
engine block manufacturer
transmission housing manufacturer
alloy wheel manufacturer
wheel hub manufacturer
crankshaft manufacturer
gearbox housing casting
aluminum casting automotive
```

**城市（全部粘贴）：**
```
Detroit
Toledo
Cleveland
Monterrey
Saltillo
Queretaro
Stuttgart
Nürnberg
Ingolstadt
Bursa
Kocaeli
Nagoya
Pune
Chennai
Rayong
Chonburi
Torino
Incheon
```

= 10 关键词 × 18 城市 = **180 条组合**

---

### P0-C：灰铁/球铁铸造厂

**关键词（全部粘贴）：**
```
iron casting foundry
gray iron foundry
ductile iron foundry
metal foundry
```

**城市（全部粘贴）：**
```
Cleveland
Toledo
Indianapolis
Milwaukee
Monclova
Ramos Arizpe
Kocaeli
Manisa
Kolhapur
Howrah
Stuttgart
Nürnberg
Bursa
Istanbul
Monterrey
Saltillo
```

= 4 关键词 × 16 城市 = **64 条组合**

---

### P0 小计：394 条组合

---

## P1 — 卫浴五金 / 水龙头

### P1-A：水龙头/花洒制造商

**关键词（全部粘贴）：**
```
faucet manufacturer
brass faucet factory
shower head manufacturer
sanitary ware manufacturer
plumbing fixture manufacturer
```

**城市（全部粘贴）：**
```
Kaiping
Yuhuan
Wenzhou
Nan'an
Xiamen
Jamnagar
Morbi
Rajkot
Lumezzane
Brescia
Bursa
Kayseri
Istanbul
```

= 5 关键词 × 13 城市 = **65 条组合**

---

### P1-B：五金门锁/把手/水槽

**关键词（全部粘贴）：**
```
door handle manufacturer
door lock manufacturer
stainless steel sink manufacturer
brass hardware manufacturer
bathroom hardware manufacturer
brass fitting manufacturer
bathroom fitting manufacturer
```

**城市（全部粘贴）：**
```
Wenzhou
Nan'an
Yuhuan
Kaiping
Jamnagar
Morbi
Rajkot
Lumezzane
Brescia
Bursa
Kayseri
Istanbul
```

= 7 关键词 × 12 城市 = **84 条组合**

---

### P1 小计：149 条组合

---

## P2 — EV电池托盘 / 新能源

**关键词（全部粘贴）：**
```
EV battery tray manufacturer
battery enclosure manufacturer
battery housing manufacturer
aluminum battery box manufacturer
EV component manufacturer
battery tray welding
```

**城市（全部粘贴）：**
```
Detroit
Lansing
Kokomo
Spring Hill
Salzgitter
Debrecen
Wroclaw
Győr
Gliwice
Seosan
Munich
Rayong
Chonburi
```

= 6 关键词 × 13 城市 = **78 条组合**

---

## P3 — 通用铸造厂

**关键词（全部粘贴）：**
```
casting foundry
gray iron foundry
ductile iron foundry
steel casting manufacturer
investment casting manufacturer
pump body casting
valve body casting
machine base casting
metal foundry
```

**城市（全部粘贴）：**
```
Kolhapur
Coimbatore
Howrah
Rajkot
Bhavnagar
Vadodara
Jalandhar
Joinville
Caxias do Sul
Monclova
Ramos Arizpe
Sibiu
Cluj
Manisa
Istanbul
Bursa
Cleveland
Pune
```

= 9 关键词 × 18 城市 = **162 条组合**

---

## P4 — 钣金 / 焊接加工厂

**关键词（全部粘贴）：**
```
sheet metal fabrication
metal fabrication shop
welding fabrication company
laser cutting service
metal stamping manufacturer
job shop metalwork
powder coating metal
```

**城市（全部粘贴）：**
```
Detroit
Monterrey
Indianapolis
Cleveland
Toledo
Queretaro
Stuttgart
Pune
Coimbatore
Rajkot
Bursa
Istanbul
Joinville
Wroclaw
Rayong
```

= 7 关键词 × 15 城市 = **105 条组合**

---

## P5 — 航空航天

**关键词（全部粘贴）：**
```
aerospace machining
aerospace component manufacturer
turbine blade manufacturer
aircraft parts manufacturer
titanium machining aerospace
aerospace precision machining
```

**城市（全部粘贴）：**
```
Wichita
Seattle
Hartford
Cincinnati
Fort Worth
Toulouse
Hamburg
Munich
Bristol
Derby
Turin
Naples
```

= 6 关键词 × 12 城市 = **72 条组合**

---

## P6 — 骨科植入物 / 医疗器械

**关键词（全部粘贴）：**
```
orthopedic implant manufacturer
medical device manufacturer
joint replacement manufacturer
surgical instrument manufacturer
medical implant machining
cobalt chrome machining
```

**城市（全部粘贴）：**
```
Warsaw
Memphis
Minneapolis
Boston
Galway
Cork
Tuttlingen
Biel
Solothurn
```

= 6 关键词 × 9 城市 = **54 条组合**

---

## 全局统计

| 分组 | 客户群体 | 关键词数 | 城市数 | 组合数 |
|---|---|---|---|---|
| P0-A | 压铸厂核心 | 5 | 30 | 150 |
| P0-B | 汽车零部件铸造 | 10 | 18 | 180 |
| P0-C | 灰铁/球铁铸造 | 4 | 16 | 64 |
| P1-A | 水龙头/花洒 | 5 | 13 | 65 |
| P1-B | 五金门锁/水槽 | 7 | 12 | 84 |
| P2 | EV电池托盘 | 6 | 13 | 78 |
| P3 | 通用铸造 | 9 | 18 | 162 |
| P4 | 钣金焊接 | 7 | 15 | 105 |
| P5 | 航空航天 | 6 | 12 | 72 |
| P6 | 医疗骨科 | 6 | 9 | 54 |
| **合计** | | **65** | **50+** | **1014** |

---

## 执行方案

### 方案 A：gosom 批量脚本（推荐，免费）

**前置条件：** 安装 Go + gosom

```bash
# 1. 安装 Go（Windows: https://go.dev/dl/ 或 winget install GoLang.Go）
# 2. 安装 gosom
go install github.com/gosom/google-maps-scraper@latest

# 3. 用批量脚本执行
cd google-maps-scraper
python batch_scraper.py --method gosom --group P0-A           # 跑单组
python batch_scraper.py --method gosom --group all            # 跑全部
python batch_scraper.py --dry-run --group all                 # 预览所有查询
python batch_scraper.py --merge --results-dir results/P0-A    # 合并去重
```

**优势：** 完全免费，自带反检测，不需要代理，自动去重

### 方案 B：Apify API（省心，约 $5-8）

```bash
# 1. 注册 https://apify.com（免费送 $5 额度）
# 2. pip install apify-client
# 3. set APIFY_API_TOKEN=your_token
# 4. python batch_scraper.py --method apify --group all
```

### 方案 C：LetsExtract（备选）

1. **操作方式**：每组关键词 + 城市直接批量粘贴到工具里，一次跑完该组全部组合
2. **每天执行量**：建议每天跑 1-2 组（避免Google封IP）
3. **优先跑的顺序**：
   - 第1天：P0-A（压铸核心，150条）
   - 第2天：P0-B（汽车零部件，180条）
   - 第3天：P1-A + P1-B（卫浴五金，149条）
   - 第4天：P2 + P3（EV + 通用铸造）
   - 第5天：P0-C + P4（灰铁铸造 + 钣金）
   - 第6天：P5 + P6（航空 + 医疗）
4. **去重**：不同组之间会有重复公司，最终合并去重
5. **Google Maps 优先**
6. **墨西哥优先**：Monterrey/Saltillo 密度最高，接受度最好
