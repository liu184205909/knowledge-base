...
function getTTSSettings() {
  ...
  return {
    language: settingsSheet.getRange("B42").getValue() || "zh-cn", // TTS语言设置
    engine: settingsSheet.getRange("B43").getValue() || "gtts",    // TTS引擎设置
    voice: settingsSheet.getRange("B44").getValue() || "default",  // 语音设置
    speed: settingsSheet.getRange("B45").getValue() || 1.0,        // 语速设置
    volume: settingsSheet.getRange("B46").getValue() || 0.9        // 音量设置
  };
}
...
function logTTSResultsToSheet(results) {
  ...
  // 为WP All Import优化列标题
  if (lastRow >= 1) {
    sheet.getRange(1, 2).setValue("audio_url");        // WP All Import字段名
  ...
  for (var i = 0; i < results.audioFiles.length; i++) {
    ...
    if (wordRow > 0) {
      // 为WP All Import兼容，直接显示URL
      sheet.getRange(wordRow, 2).setValue(audioFile.url);
      
      // 设置格式
      sheet.getRange(wordRow, 2).setFontColor("#1a73e8")
                                .setFontStyle("italic");
    }
  }
  ...
  for (var i = 0; i < results.failedWords.length; i++) {
    ...
    if (wordRow > 0) {
      sheet.getRange(wordRow, 2).setValue(""); // 空的URL
    }
  }
}
