...
function onOpen() {
    ...
    ui.createMenu('🎉 Generate')
        ...
        .addSeparator()
        .addItem("🎵 批量TTS转换", "batchTTSConversion")
        .addItem("🎵 单词转音频", "wordToAudio")
        .addSeparator()
        .addItem("🔗 批量转换直接下载链接", "batchConvertToDirectDownload")
        .addToUi();
  }
  ...
  function fetchGlobalSettings(settingsSheet) {
    return {
      ...
      // DeepSeek设置
      deepseekApiKey: settingsSheet.getRange("B13").getValue(),
      deepseekTemperature: Number(settingsSheet.getRange("B14").getValue()),
      deepseekMaxTokens: Number(settingsSheet.getRange("B15").getValue()),
      deepseekSystemPrompt: settingsSheet.getRange("B16").getValue(),
      
      // Kimi设置
      kimiApiKey: settingsSheet.getRange("B17").getValue(),
      kimiTemperature: Number(settingsSheet.getRange("B18").getValue()),
      kimiMaxTokens: Number(settingsSheet.getRange("B19").getValue()),
      kimiSystemPrompt: settingsSheet.getRange("B20").getValue(),
      
      // Zhipu设置
      zhipuApiKey: settingsSheet.getRange("B21").getValue(),
      zhipuTemperature: Number(settingsSheet.getRange("B22").getValue()),
      zhipuMaxTokens: Number(settingsSheet.getRange("B23").getValue()),
      zhipuSystemPrompt: settingsSheet.getRange("B24").getValue(),
      
      // --- Imagen settings ---
      imagenSampleCount: Number(settingsSheet.getRange("B25").getValue()), // 1-4
      imagenAspectRatio: settingsSheet.getRange("B26").getValue(),         // "1:1"|"3:4"|"4:3"|"9:16"|"16:9"
      imagenSize: settingsSheet.getRange("B27").getValue(),                // "1K"|"2K"（Ultra/Standard 支持）
      imagenPersonGen: settingsSheet.getRange("B28").getValue(),           // "allow_adult"|"dont_allow"
      driveFolderId: settingsSheet.getRange("B29").getValue()              // 可选：生成图保存的 Drive 文件夹ID
    };
  }
...
function sendToImagen(prompt, apiKey, model, opts) {
  ...
  try {
    ...
    var viewUrl = "https://drive.google.com/thumbnail?id=" + file.getId() + "&sz=w1000";
    // 返回可直接嵌入的URL，适合WordPress导入
    return viewUrl;
  } catch (err) {
  ...
  function sendToDeepSeek(prompt, apiKey, model, temperature, maxTokens, systemPrompt) {
    ...
    // 根据文档优化：deepseek-chat 和 deepseek-reasoner 都基于 DeepSeek-V3.2-Exp
    var deepSeekData = {
      messages: [
    ...
    try {
      ...
      if (response.getResponseCode() !== 200) {
        Logger.log('DeepSeek API Error: ' + response.getContentText());
        return 'Error: DeepSeek API request failed - ' + (jsonResponse.error ? jsonResponse.error.message : 'Unknown error');
      }
      
      if (jsonResponse && jsonResponse.choices && jsonResponse.choices[0] && jsonResponse.choices[0].message) {
        return jsonResponse.choices[0].message.content;
      } else {
        Logger.log('Unexpected DeepSeek response structure: ' + response.getContentText());
        return 'Error: Unexpected response structure from DeepSeek';
      }
    } catch (error) {
      Logger.log('Error sending to DeepSeek: ' + error.toString());
      return 'Error: Failed to fetch response from DeepSeek - ' + error.toString();
    }
  }
  ...
  function sendToZhipu(prompt, apiKey, model, temperature, maxTokens, systemPrompt) {
    ...
    try {
      ...
      if (jsonResponse && jsonResponse.choices && jsonResponse.choices[0].message && jsonResponse.choices[0].message.content) {
        return jsonResponse.choices[0].message.content;
      } else {
        Logger.log('Unexpected Zhipu response structure: ' + response.getContentText());
        return 'Error: Unexpected response structure from Zhipu AI';
      }
    } catch (error) {
      Logger.log('Error sending to Zhipu AI: ' + error);
      return 'Error: Failed to fetch response from Zhipu AI';
    }
  }
...
function KIMI(prompt, model, temperature, maxTokens, systemPrompt) {
  ...
  var mdl = String(model || 'moonshot-v1-32k'); // 直接使用默认模型moonshot-v1-32k
...
function CLAUDE(prompt, model, temperature, maxTokens, systemPrompt) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var s = fetchGlobalSettings(ss.getSheetByName('Settings'));
  var mdl = String(model || 'claude-3-5-sonnet-latest'); // 直接使用默认模型
...
function GEMINI(prompt, model, temperature, maxTokens) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var s = fetchGlobalSettings(ss.getSheetByName('Settings'));
  var mdl = String(model || 'gemini-2.5-flash'); // 直接使用默认模型
...
function DEEPSEEK(prompt, model, temperature, maxTokens, systemPrompt) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var s = fetchGlobalSettings(ss.getSheetByName('Settings'));
  var mdl = String(model || 'deepseek-chat'); // 直接使用默认模型，不从Settings读取
...
function ZHIPU(prompt, model, temperature, maxTokens, systemPrompt) {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var s = fetchGlobalSettings(ss.getSheetByName('Settings'));
    var mdl = String(model || 'glm-4.6'); // 直接使用默认模型
...
/**
 * 批量TTS转换主函数
 * 从当前选中的工作表读取单词列表并批量转换为音频文件
 */
function batchTTSConversion() {
  var ui = SpreadsheetApp.getUi();
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  ...
  try {
    ...
    if (words.length === 0) {
      ui.alert("错误", "没有找到有效的单词数据！请确保A列包含单词内容。", ui.ButtonSet.OK);
      return;
    }
    ...
    if (response === ui.Button.YES) {
      ...
      showTTSResults(results);
    }
    
  } catch (error) {
    ui.alert("错误", "TTS转换失败: " + error.toString(), ui.ButtonSet.OK);
    Logger.log("TTS转换错误: " + error.toString());
  }
}
...
function wordToAudio() {
  ...
  if (range.getNumRows() !== 1 || range.getNumColumns() !== 1) {
    ui.alert("错误", "请选择一个单元格！", ui.ButtonSet.OK);
    return;
  }
  
  var word = range.getValue().toString().trim();
  
  if (!word) {
    ui.alert("错误", "所选单元格为空！", ui.ButtonSet.OK);
    return;
  }
  
  try {
    var settings = getTTSSettings();
    var results = convertWordsToAudio([word], settings);
    showTTSResults(results);
  } catch (error) {
    ui.alert("错误", "单词转换失败: " + error.toString(), ui.ButtonSet.OK);
  }
}
...
function getTTSSettings() {
  ...
  if (!settingsSheet) {
    throw new Error("找不到Settings工作表");
  }
  
  return {
    language: settingsSheet.getRange("B36").getValue() || "zh-cn", // TTS语言设置
    engine: settingsSheet.getRange("B37").getValue() || "gtts",    // TTS引擎设置
    voice: settingsSheet.getRange("B38").getValue() || "default",  // 语音设置
    speed: settingsSheet.getRange("B39").getValue() || 1.0,        // 语速设置
    volume: settingsSheet.getRange("B40").getValue() || 0.9        // 音量设置
  };
}
 ...
 * 从工作表中获取单词列表
 */
function getWordListFromSheet(sheet) {
 ...
 * 转换单词列表为音频
 */
function convertWordsToAudio(words, settings) {
  var results = {
    total: words.length,
    success: 0,
    failed: 0,
  ...
  for (var i = 0; i < words.length; i++) {
    ...
    try {
      var audioFile = convertSingleWordToAudio(word, settings);
      if (audioFile) {
        results.success++;
        results.audioFiles.push({
          ...
          url: audioFile.url,
          directDownloadUrl: audioFile.directDownloadUrl
        });
        Logger.log(`成功转换: ${word} -> ${audioFile.name}`);
      } else {
        results.failed++;
        results.failedWords.push(word);
        Logger.log(`转换失败: ${word}`);
      }
    } catch (error) {
      results.failed++;
      results.failedWords.push(word);
      Logger.log(`转换错误 ${word}: ${error.toString()}`);
    }
 ...
 * 转换单个单词为音频
 */
function convertSingleWordToAudio(word, settings) {
  var audioContent;
  
  if (settings.engine === "gtts") {
    audioContent = convertWithGTTS(word, settings);
  } else if (settings.engine === "azure") {
    audioContent = convertWithAzure(word, settings);
  } else {
    throw new Error("不支持的TTS引擎: " + settings.engine);
  }
  ...
  // 保存到Google Drive
  var filename = sanitizeFilename(word) + ".mp3";
  var blob = Utilities.newBlob(audioContent, "audio/mpeg", filename);
  
  // 获取或创建TTS音频文件夹
  var folder = getOrCreateTTSFolder();
 ...
 * 使用Google TTS转换
 */
function convertWithGTTS(word, settings) {
  try {
    var url = "https://translate.google.com/translate_tts?ie=UTF-8&tl=" + 
              settings.language + "&client=tw-ob&q=" + encodeURIComponent(word);
    ...
    Logger.log("gTTS转换错误: " + error.toString());
    return null;
  }
}

/**
 * 使用Azure TTS转换（需要API密钥）
 */
function convertWithAzure(word, settings) {
  // 这里可以集成Azure Speech Services
  // 需要配置Azure API密钥
  throw new Error("Azure TTS暂未实现，请使用gTTS引擎");
}
 ...
 * 获取或创建TTS音频文件夹
 */
function getOrCreateTTSFolder() {
  var folderName = "TTS音频文件";
 ...
 * 显示TTS转换结果
 */
function showTTSResults(results) {
  var ui = SpreadsheetApp.getUi();
  
  var message = `TTS转换完成！\n\n` +
                `总单词数: ${results.total}\n` +
                `成功转换: ${results.success}\n` +
                `失败数量: ${results.failed}\n\n` +
                `音频文件已保存到Google Drive的"TTS音频文件"文件夹中。`;
  
  if (results.failedWords.length > 0) {
    message += `\n\n失败的单词:\n${results.failedWords.join(", ")}`;
  }
  
  ui.alert("转换结果", message, ui.ButtonSet.OK);
  
  // 可选：在工作表中显示结果
  logTTSResultsToSheet(results);
}

/**
 * 将Google Drive查看链接转换为直接下载链接
 * @param {string} viewUrl - Google Drive查看链接
 * @return {string} - 直接下载链接
 */
function convertToDirectDownloadUrl(viewUrl) {
  if (!viewUrl || viewUrl.trim() === "") {
    return "";
  }
  
  // 使用正则表达式提取文件ID
  var fileIdMatch = viewUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);
  if (fileIdMatch && fileIdMatch[1]) {
    return "https://drive.google.com/uc?export=download&id=" + fileIdMatch[1];
  }
  
  // 如果已经是直接下载链接格式，直接返回
  if (viewUrl.includes("/uc?export=download&id=")) {
    return viewUrl;
  }
...
/**
 * 将TTS结果记录到工作表 - 在单词后面的行中显示音频链接、状态和文件名
 */
function logTTSResultsToSheet(results) {
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = spreadsheet.getActiveSheet();
  ...
  if (lastRow >= 1) {
    // B列：音频URL
    sheet.getRange(1, 2).setValue("audio_url");        
    sheet.getRange(1, 2).setBackground("#e8f0fe")
                               .setFontWeight("bold")
                               .setHorizontalAlignment("center");
    
    // CQ列：TTS状态（如果列存在）
    var cqColumn = findColumnByHeader(sheet, "TTS状态") || 95; // CQ列默认是第95列
    if (cqColumn > 0) {
      sheet.getRange(1, cqColumn).setValue("TTS状态");
      sheet.getRange(1, cqColumn).setBackground("#fff2cc")
                                 .setFontWeight("bold")
                                 .setHorizontalAlignment("center");
    }
    
    // CR列：音频文件（如果列存在）
    var crColumn = findColumnByHeader(sheet, "音频文件") || 96; // CR列默认是第96列
    if (crColumn > 0) {
      sheet.getRange(1, crColumn).setValue("音频文件");
      sheet.getRange(1, crColumn).setBackground("#fff2cc")
                                 .setFontWeight("bold")
                                 .setHorizontalAlignment("center");
    }
  }
  
  // 为每个成功的音频文件在工作表中添加完整信息
  for (var i = 0; i < results.audioFiles.length; i++) {
    var audioFile = results.audioFiles[i];
    
    // 找到对应的单词行
    var wordRow = findWordRow(sheet, audioFile.word);
    
    if (wordRow > 0) {
      // B列：音频URL（直接下载链接）
      var downloadUrl = audioFile.directDownloadUrl || convertToDirectDownloadUrl(audioFile.url);
      sheet.getRange(wordRow, 2).setValue(downloadUrl);
      sheet.getRange(wordRow, 2).setFontColor("#1a73e8").setFontStyle("italic");
      
      // CQ列：TTS状态
      var cqColumn = findColumnByHeader(sheet, "TTS状态") || 95;
      if (cqColumn > 0) {
        sheet.getRange(wordRow, cqColumn).setValue("√ 成功");
        sheet.getRange(wordRow, cqColumn).setFontColor("#0f9d58").setFontWeight("bold");
      }
      
      // CR列：音频文件名
      var crColumn = findColumnByHeader(sheet, "音频文件") || 96;
      if (crColumn > 0) {
        sheet.getRange(wordRow, crColumn).setValue(audioFile.name);
        sheet.getRange(wordRow, crColumn).setFontColor("#333333");
      }
    }
  }
  
  // 为失败的单词添加状态
  for (var i = 0; i < results.failedWords.length; i++) {
    var failedWord = results.failedWords[i];
    var wordRow = findWordRow(sheet, failedWord);
    
    if (wordRow > 0) {
      // B列：空的URL
      sheet.getRange(wordRow, 2).setValue("");
      
      // CQ列：失败状态
      var cqColumn = findColumnByHeader(sheet, "TTS状态") || 95;
      if (cqColumn > 0) {
        sheet.getRange(wordRow, cqColumn).setValue("✗ 失败");
        sheet.getRange(wordRow, cqColumn).setFontColor("#ea4335").setFontWeight("bold");
      }
      
      // CR列：空的文件名
      var crColumn = findColumnByHeader(sheet, "音频文件") || 96;
      if (crColumn > 0) {
        sheet.getRange(wordRow, crColumn).setValue("");
      }
    }
  }
}
...
function findWordRow(sheet, word) {
  ...
  for (var i = 1; i < data.length; i++) { // 跳过标题行
    if (data[i][0] && data[i][0].toString().trim() === word) {
 ...
 * @param {string} headerText - 列标题文本
 * @return {number} - 列号（1-based），如果未找到返回-1
 */
function findColumnByHeader(sheet, headerText) {
  ...
  for (var i = 0; i < headers.length; i++) {
    if (headers[i] && headers[i].toString().trim() === headerText) {
      return i + 1; // 返回1-based列号
    }
  }
...
/**
 * 智能批量转换Google Drive链接为直接下载链接
 * 自动检测链接类型，只转换需要转换的链接
 */
function batchConvertToDirectDownload() {
  var ui = SpreadsheetApp.getUi();
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = spreadsheet.getActiveSheet();
  ...
  try {
    var lastRow = sheet.getLastRow();
    if (lastRow <= 1) {
      ui.alert("错误", "工作表中没有数据！", ui.ButtonSet.OK);
      return;
    }
    
    var convertedCount = 0;
    var alreadyDirectCount = 0;
    var errorCount = 0;
    var emptyCount = 0;
    ...
    for (var i = 2; i <= lastRow; i++) {
      ...
      var url = cellValue.toString().trim();
      
      // 检查是否已经是直接下载链接
      if (url.includes("/uc?export=download&id=")) {
        alreadyDirectCount++;
        // 设置格式表示这是直接下载链接
        sheet.getRange(i, 2).setFontColor("#0f9d58").setFontStyle("bold");
        continue;
      }
      
      // 检查是否是Google Drive查看链接
      if (url.includes("drive.google.com/file/d/")) {
        var directUrl = convertToDirectDownloadUrl(url);
        
        if (directUrl) {
          sheet.getRange(i, 2).setValue(directUrl);
          sheet.getRange(i, 2).setFontColor("#1a73e8").setFontStyle("italic");
          convertedCount++;
        } else {
          errorCount++;
        }
      } else {
        errorCount++; // 非Google Drive链接
      }
    }
    
    var message = `智能转换完成！\n\n` +
                  `✅ 新转换: ${convertedCount} 个链接\n` +
                  `✅ 已是直接链接: ${alreadyDirectCount} 个\n` +
                  `❌ 转换失败: ${errorCount} 个\n` +
                  `⚪ 空单元格: ${emptyCount} 个\n\n` +
                  `绿色=直接下载链接 | 蓝色=新转换链接`;
    ...
    ui.alert("错误", "批量转换失败: " + error.toString(), ui.ButtonSet.OK);
    Logger.log("批量转换错误: " + error.toString());
  }
}
