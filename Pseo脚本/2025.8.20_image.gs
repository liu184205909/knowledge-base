...
function onOpen() {
  ...
  function fetchGlobalSettings(settingsSheet) {
    return {
      ...
      // --- 智谱AI settings ---
      zhipuApiKey: settingsSheet.getRange("B38").getValue(),               // 智谱AI API密钥
      zhipuImageModel: settingsSheet.getRange("B39").getValue(),           // 图像生成模型，如 "cogview-4-250304"
      zhipuImageSize: settingsSheet.getRange("B40").getValue(),            // 图像尺寸，如 "1024x1024"
      zhipuImageQuality: settingsSheet.getRange("B41").getValue(),         // 图像质量，如 "standard" 或 "hd"
      zhipuImageStyle: settingsSheet.getRange("B42").getValue()            // 图像风格，如 "vivid" 或 "natural"
    };
  }
