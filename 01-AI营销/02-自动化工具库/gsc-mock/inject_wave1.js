(() => {
  try {
    ...
    const hits = { text: 0, icon: 0, domain: 0 };
    const swapPath = function (svg, d) {
    ...
    const fix = function (v) { return v.split('electricalcabinet.net').join('takusushibar.com'); };
    document.querySelectorAll('*').forEach(function (el) {
      ['placeholder', 'title', 'aria-label', 'alt'].forEach(function (a) {
        const v = el.getAttribute && el.getAttribute(a);
        if (v && v.indexOf('electricalcabinet') >= 0) { el.setAttribute(a, fix(v)); hits.domain++; }
      });
    });
    const tw2 = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
    while ((n = tw2.nextNode())) {
      const v = n.nodeValue;
      if (v && v.indexOf('electricalcabinet.net') >= 0) { n.nodeValue = fix(v); hits.domain++; }
    }
