(() => {
  try {
    ...
    const log = { applied: 0, iconsNow: 0, hiddenPanels: 0 };
    ...
    const ligMap = {
      'home': 'analytics', 'lightbulb_2': 'trending_up', 'google': 'explore',
      'file_copy': 'bookmarks', 'account_tree': 'device_hub', 'visibility_off': 'block'
    };
    const iconPaths = {
    ...
    const replaceIcon = function (labelNode, key) {
      let c = (labelNode && typeof labelNode.getBoundingClientRect === 'function') ? labelNode : (labelNode ? labelNode.parentElement : null);
      if (!c) return false;
      const lbox = c.getBoundingClientRect();
      for (let i = 0; i < 8 && c; i++) {
        if (typeof c.querySelectorAll !== 'function') { c = c.parentElement; continue; }
        const svgs = [...c.querySelectorAll('svg')].filter(function (s) {
        ...
        if (svgs.length) {
          ...
          all.forEach(function (s) {
            if (s.getAttribute('data-mine') === '1') { mine = s; }
    ...
    const applyAll = function () {
      ...
      while ((n = tw.nextNode())) {
        ...
        if (vmap[t] !== undefined) {
          ...
          n.nodeValue = n.nodeValue.split('electricalcabinet.net').join('takusushibar.com').split('electricalcabinet').join('takusushibar');
        } else if (sideMap[t] !== undefined) {
          const anchor = n;
          n.nodeValue = n.nodeValue.replace(t, sideMap[t]);
          if (iconPaths[sideMap[t]]) replaceIcon(anchor, sideMap[t]);
        } else if (iconPaths[t] !== undefined) {
          replaceIcon(n, t);
        } else if (ligMap[t] !== undefined) {
          n.nodeValue = n.nodeValue.replace(t, ligMap[t]);
        }
      }
      }
      document.documentElement.style.zoom = '0.74';
      if (!log.chart) { try { log.chart = redrawChart(); } catch (e) {} }
      ...
      document.querySelectorAll('*').forEach(function (el) {
        ['placeholder', 'title', 'aria-label', 'alt'].forEach(function (a) {
          ...
          if (v && v.indexOf('electricalcabinet') >= 0) {
            el.setAttribute(a, v.split('electricalcabinet.net').join('takusushibar.com').split('electricalcabinet').join('takusushibar'));
          }
        });
      });
      document.querySelectorAll('*').forEach(function (el) {
        if (el.children.length < 30 && (el.textContent || '').indexOf('高级可视化') >= 0 && (el.textContent || '').length < 300) {
          let c = el;
          for (let i = 0; i < 6 && c; i++) {
            const st = getComputedStyle(c);
            if (st.position === 'fixed' || st.position === 'absolute') { c.style.display = 'none'; log.hiddenPanels++; return; }
            c = c.parentElement;
          }
        }
      });
    ...
    applyAll();
    log.chart = log.chart || redrawChart();
    document.documentElement.style.zoom = '0.74';

    /* MutationObserver 持续守护 */
    let timer = null;
    const onMut = function () {
      if (timer) return;
      timer = setTimeout(function () { timer = null; applyAll(); }, 80);
    };
    const obs = new MutationObserver(onMut);
    obs.observe(document.body, { subtree: true, childList: true });
    window.__mockObserver = obs;
    const watchShadow = function () {
      for (const r of collectRoots()) {
        if (!r.__mockObs) { r.__mockObs = new MutationObserver(onMut); r.__mockObs.observe(r, { subtree: true, childList: true }); }
      }
    };
    window.__mockTimer = setInterval(function () { watchShadow(); applyAll(); }, 300);
    setTimeout(function () { clearInterval(window.__mockTimer); }, 25000);

    return JSON.stringify({ chart: log.chart, applied: log.applied, observer: 'installed' });
  } catch (e) {
