...
## 18. 实战参考：archive 模板真实结构（2026-06-30 horoscope archive id=48124）

> 用户在 Elementor 编辑器手动搭建的真实归档模板结构，与上方 REST 规范有几处出入。**施工时以用户实际做法为准**，REST 注入参考本节。

### 18.1 用户真实施工习惯（与 §3/§4/§5 规范的出入）

| # | 规范说的 | 用户实际做的 | 结论 |
|---|---------|------------|------|
| 1 | 文章列表靠主题默认 loop | 用 `posts` widget（Elementor Pro Posts）配 query | REST 注入文章列表区一律用 `posts` widget，不假设主题自动渲染 |
| 2 | 大段 HTML 用 `html` widget | 放 `text-editor` widget 承载 HTML | text-editor 富文本可装任意 HTML，编辑器更易改，优先 text-editor |
| 3 | 多列用 `flex_direction:"row"` + `flex_wrap` + 子 width% | `container[col] + container[col]` 两个 isInner container 同级并列 | 两种都可；用户偏并列 col，靠各自 width 自然分列 |
| 4 | 不用 `wd_title`，用标准 `heading` | archive 顶部页标题用 `wd_title`（WoodMart 组件） | 归档页页标题用户接受 wd_title |
| 5 | section margin-top -40 | 同（margin top:-40），padding 用 {top:"100",bottom:"100"} 大间距 | 大上下间距是用户偏好 |

### 18.2 归档模板 REST 注入结构参考

```
container[column] → wd_title                      （页标题）
container[column] → 左右双 col：
  ├─ container[col] → heading + text-editor       （区块标题 + 说明文案）
  └─ container[col] → posts                        （文章列表，query=post+category）
container[column] → text-editor                    （底部 SEO 内容，承载 HTML，常青不绑年份）
```

### 18.3 category 归档 vs CPT 归档（Elementor Archive condition 关键区别）

- **CPT 归档**（如 gemstone）：Elementor Archive condition `include/archive/{post_type}_archive` 能精确命中 → 干净。
- **category 归档**（如 horoscope，是 post 的 taxonomy term）：Elementor Archive condition 只能命中「All Archives / {CPT}_archive」，**无法精确命中单个 category term**。给单个 category 做 Archive 模板需在编辑器 Display Conditions 里手动设（用户 UI 操作），REST 写 `_elementor_conditions` 格式不确定，不建议 REST 设 condition。

### 18.4 归档页内容常青原则

归档页（永久固定链接 `/category/.../`）的 SEO intro/H1/底部文案**必须常青、不绑年份**（写死"2026"到 2027 就过时）。年份词（"2026 horoscope"）交给年度/月度 hub post 承担（每年/月新文章）。归档页网格按年分组，每年新增 hub 只加卡片，intro 永不改。见 memory `archive-evergreen-vs-yearly-hub`。
