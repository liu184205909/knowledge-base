...
> **接力关系**：本文件管发布前单篇准入；**存量内容的批量审计与优化**见同目录 [04-内容审计与优化工具包.md](./04-内容审计与优化工具包.md)（100 分评分体系 + 4 周优化冲刺 + 成熟度模型附录）。
> **来源**：基于 Simon（赛门快跑）分享的 Chrome Ask Gemini 提示词，适配 Claude Code 本地文件评估场景。
> **适用阶段**：内容生产完成后 → 发布前质检
> **EEAT 三档定义（2026-09-23 增补，接住 02-SOP §4.1 路由表）**：
> - **加强**（对比/选购类必用）：先按标准档跑评分，再附加——①附 1-3 个竞品 URL 做逐维度对比点评 ②差异化声明逐条核验（DREAM 五维）③行动清单必须含"竞品没有而本篇有"的至少 1 条
> - **标准**（百科/教程类）：完整跑本文件评分流程（评分+最弱部分+行动清单）
> - **轻量**（转化型页面）：仅跑评分 + 一句话结论，跳过逐维度展开
>
> **与 02-SOP §3.1 的关系**：§3.1 的 Marie Haynes prompt 是**作者写作后自查初筛**（作者视角），本文件是**发布前终审**（审核员视角）——批量生产先过 §3.1 自查，发布前按路由表档位过本关卡。
>
> **关卡前移规则**（2026-08-26 增补，批量场景）：单篇逐评只适用于小批量；≥20 篇批量生产时改为——①每类模板首篇必跑（>4 分修模板再放量）②放量后每类型抽 10% 复评监测模板漂移 ③单篇 ≥5 分触发线：暂停该模板产出，补强差异化资产后复评。本次逐篇后置会滞后于批量生产节奏
...
1. **发布前必检**：每篇文章发布前跑一次，评分 ≤ 4 才建议发布（批量场景按头部"关卡前移规则"执行：模板首篇必跑 + 10% 抽检）
