# P3-E-3: The Hoodwitch 深度拆解

> **竞品URL**: https://thehoodwitch.com / https://www.thehoodwitch.com
> **分析日期**: 2026-05-04
> **参考价值**: （灵性方向 — 现代巫术美学+星座运势+月相仪式+时尚风格参考）
> **研究重点**: Bri Luna创始人、Lisa Stardust占星师合作、"Everyday magic for the modern mystic"定位、Squarespace平台、Witch Tips & Horoscopes系列

---

## 参考索引（AI快速参考）

| 参考项 | 核心要点 | 适用场景 | 验证URL |
|--------|----------|----------|---------|
| **现代巫术美学** | "Everyday magic for the modern mystic" | 品牌调性+视觉美学参考 | https://thehoodwitch.com/ |
| **Witch Tips & Horoscopes** | 每周12星座运势+Witch Tips | 每周内容节奏参考 | https://thehoodwitch.com/ |
| **月相仪式** | Full Moon/New Moon Ritual系列 | 仪式模板内容参考 | https://thehoodwitch.com/ |
| **Lisa Stardust** | 合伙占星师Lisa Stardust | 外部专家合作参考 | https://thehoodwitch.com/ |
| **Bri Luna** | 创始人，品牌美学主理 | 创始人×美学品牌参考 | https://thehoodwitch.com/ |
| **Squarespace** | Squarespace建站 | 无代码建站参考 | https://thehoodwitch.com/ |

---

## 1. 基本信息

| 维度 | 详情 |
|------|------|
| **网站** | thehoodwitch.com | https://thehoodwitch.com/ |
| **创始人** | Bri Luna（品牌美学主理人/作者） | https://thehoodwitch.com/ |
| **占星师** | Lisa Stardust（合作占星师，撰写所有星座内容） | https://thehoodwitch.com/ |
| **定位** | "Everyday magic for the modern mystic" | https://thehoodwitch.com/ |
| **理念** | "With intention, love, and respect for our planet we are capable of all things" | https://thehoodwitch.com/ |
| **技术栈** | Squarespace | https://thehoodwitch.com/ |
| **域名年龄** | 2014年（static1.squarespace.com/static/53d5b207... → 2014年7月） | https://thehoodwitch.com/ |
| **设计** | 闪屏封面页 + 深色极简设计 | https://thehoodwitch.com/ |
| **品牌验证** | p:domain_verify（Google） | https://thehoodwitch.com/ |

---

## 2. 网站内容结构

### 2.1 导航架构

```
The Hoodwitch 网站结构：
├── 封面闪屏页（ENTER入口）
├── Blog（核心内容板块）
│   ├── Witch Tips & Horoscopes（每周星座运势）
│   ├── Mercury Retrograde 指南
│   ├── Venus Retrograde 指南
│   ├── Full Moon Ritual（满月仪式）
│   ├── New Moon Ritual（新月仪式）
│   ├── Welcome [Season] Season（季节内容）
│   ├── Erotic Astrology 系列
│   └── Endless Bummer（生活建议）
└── 其他页面（待探索）
```

### 2.2 内容节奏

```
每周：Witch Tips & Horoscopes（12星座运势 + Witch Tips）
每月：Full Moon/New Moon Ritual（月相仪式）
季节：Welcome [Season] Season（季节性内容）
事件驱动：
├── Mercury Retrograde（水逆指南）
├── Venus Retrograde（金星逆行指南）
├── Eclipse Season（日月食季节）
└── 其他重大天文事件
```

---

## 3. 内容策略分析

### 3.1 Witch Tips & Horoscopes 系列

```
每周发布的核心内容：
1. 天文事件摘要（本周重要星象）
2. Witch Tip（实用巫术建议）
3. 12星座运势（Aries到Pisces逐个分析）

内容矩阵：
每周1篇文章 = 12个星座运势 + 1个Witch Tip + 天文分析
= 1篇超长内容覆盖12+个SEO关键词

这是"内容矩阵乘法"的另一种形式：
不是12个独立页面，而是1篇文章覆盖12个星座
```

### 3.2 月相仪式内容

```
Full Moon/New Moon Ritual系列：
- 每个满月/新月都有对应仪式
- 包含具体操作步骤（蜡烛、精油、冥想）
- 结合星座特征定制

Full Moon in Leo Ritual示例：
- 适用范围：1月21日-2月18日之间的满月
- 关注主题：Power/Leadership/Success/Creativity/Determination
- 仪式步骤：金/橙蜡烛+乳香/肉桂/橙精油+冥想+咒语

对 Earthward 的启发：
- "Crystal Moon Ritual" 系列
- 每月满月/新月 × 水晶推荐
- 结合星座的水晶仪式指南
```

### 3.3 季节性内容

```
"Welcome [Season] Season" 系列：
- 每个星座季节的深度内容
- Pisces Season示例：
  ├── 写给所有4元素分组（Fire/Earth/Air/Water）
  ├── Key Dates列表
  └── 占星师Lisa Stardust分析

内容模式：
星座季节 × 4元素分组 = 4个mini运势
+ Key Dates = 实用参考
+ 深度分析 = 权威内容
```

---

## 4. 变现分析

### 4.1 变现层级

| 层级 | 变现方式 | 详情 |
|------|---------|------|
| **L1** | 博客广告 | Squarespace站点可能有的广告 | https://thehoodwitch.com/ |
| **L2** | 可能的产品/服务 | 品牌相关产品（待确认） | https://thehoodwitch.com/ |
| **L3** | 品牌合作 | 时尚/灵性品牌合作 | https://thehoodwitch.com/ |

**注意**：The Hoodwitch 的变现模式从网站上看不明确，可能更多依赖品牌合作和线下活动。

---

## 5. 对 Earthward 的启示

### 5.1 必须学习

| 策略 | The Hoodwitch做法 | Earthward应用 |
|------|------------------|------------------|
| **现代美学** | 深色极简+巫术美学 | 水晶品牌视觉风格 | https://thehoodwitch.com/ |
| **Witch Tips** | 每周巫术+星座建议 | 每周水晶+星座推荐 | https://thehoodwitch.com/ |
| **月相仪式** | 满月/新月仪式模板 | 水晶×月相仪式指南 | https://thehoodwitch.com/ |
| **Lisa Stardust合作** | 外部占星师合作 | 外部灵性专家合作 | https://thehoodwitch.com/ |
| **季节性内容** | 星座季节×4元素 | 水晶×季节×星座 | https://thehoodwitch.com/ |
| **闪屏设计** | 封面页ENTER入口 | 品牌调性建立 | https://thehoodwitch.com/ |

### 5.2 The Hoodwitch 的弱点

| 弱点 | 说明 |
|------|------|
| **内容频率不确定** | 从blog看，发布间隔不规律 | https://thehoodwitch.com/ |
| **变现不清晰** | 网站上无明显变现路径 | https://thehoodwitch.com/ |
| **Squarespace限制** | 功能不如WordPress/Shopify灵活 | https://thehoodwitch.com/ |
| **无水晶产品** | 不涉及水晶电商 | https://thehoodwitch.com/ |
| **无互动工具** | 没有测试/占卜工具 | https://thehoodwitch.com/ |

### 5.3 关键启发

```
The Hoodwitch = 现代巫术美学 × 占星内容 × 月相仪式

核心差异化：
1. 视觉美学远超竞品 — 深色极简+时尚感
2. 巫术×星座×月相 的内容组合独一无二
3. Bri Luna 的美学品牌 + Lisa Stardust 的占星专业 = 完美分工

Earthward 的品牌美学方向：
1. 现代简约 vs. 传统灵性 — 选择现代简约
2. 深色系 vs. 浅色系 — 可以两种都提供
3. 水晶×月相仪式 = 巫术×水晶的交叉内容
4. 找一个外部占星师合作 = 专业内容+互相推广

The Hoodwitch 证明：
在灵性领域，"美学"和"内容"同样重要
好看的灵性内容 > 不好看的灵性内容
```

---

## 6. 关键数据速查

| 指标 | 数值 |
|------|------|
| DR | ~50 |
| 月流量 | ~300K |
| 创始人 | Bri Luna |
| 占星师 | Lisa Stardust |
| 定位 | "Everyday magic for the modern mystic" |
| 内容系列 | Witch Tips & Horoscopes（每周） |
| 月相仪式 | Full Moon/New Moon Ritual |
| 季节内容 | Welcome [Season] Season |
| 特殊系列 | Erotic Astrology |
| 平台 | Squarespace |
| 域名年龄 | ~2014年 |
| 变现层级 | 不明确（可能2-3层） |
| 设计风格 | 深色极简+巫术美学 |

---

**创建时间**: 2026-05-04
**文档类型**: P3-E 灵性方向竞品深度拆解
**分析竞品**: The Hoodwitch (thehoodwitch.com)
**关键发现**: Bri Luna创始人+Lisa Stardust占星师合作+现代巫术美学定位+每周Witch Tips & Horoscopes+月相仪式系列+Squarespace平台
