...
- **推荐方案**: B1b Crystal Single作为 canonical SEO百科页，使用 `/crystal-guide/[crystal]-meaning`；步骤3中对应的 Crystal Meaning博客选题改为支撑/扩展文章，避免争夺同一主关键词。
- **备选方案**: 取消B1b Crystal Single批量页，保留Gutenberg博客作为 canonical meaning页；`crystal-single.js` 仅作为未来百科模板参考。
...
搜索结果可以确认其存在水晶meaning页面，但命令行HTTP请求容易被拦截或返回非标准状态。进入2C抓取前应使用浏览器逐个打开并记录实际URL，而不是继续沿用 `moonrisecrystals.com/crystal-guides/[crystal]` 这类占位路径。
...
| B1b Crystal Single 2C | 暂缓批量生成 | URL归属冲突、脚本自动执行、部分竞品URL缺口、图片复用风险 |
| B1b Intention Category 2C | 暂缓批量生成 | Energy Muse模板URL不可用、命名不统一、脚本自动执行、图片占位风险 |
| B1b准备工作 | 可以立即推进 | 可先改脚本、定URL、补竞品URL、做配置和图片prompt清单 |
