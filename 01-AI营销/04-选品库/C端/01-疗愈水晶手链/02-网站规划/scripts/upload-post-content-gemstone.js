 ...
 *     images: { featured:{file,alt}, overview:{...}, properties, benefits, how_to_use, pairings }
 *   }
 *
 * Image placeholders in content (replaced on --upload-images):
 *   {KEY}_IMAGE_URL      -> real media URL
 *   wp-image-{KEY}_ID    -> wp-image-{real_id}
 *   KEY = FEATURED | OVERVIEW | PROPERTIES | BENEFITS | HOW_TO_USE | PAIRINGS
 *
 * Usage:
...
const IMAGE_PLACEHOLDERS = {
  ...
  pairings:   { urlToken: 'PAIRINGS_IMAGE_URL',   idToken: 'wp-image-PAIRINGS_ID' }
};
...
function parseArgs() {
  ...
  let status = null;
  if (args.includes('--publish')) status = 'publish';
...
async function findExistingGemstone(slug) {
  const items = await E.apiRequest('/wp-json/wp/v2/gemstone?slug=' + encodeURIComponent(slug) + '&context=edit', 'GET');
...
async function main() {
  ...
  const payload = {
    title: data.title,
    slug: data.slug,
    status: opts.status, // 'draft' | 'publish'
    content,
    excerpt: data.excerpt
  };
