/**
 * 合并项 AI 批量填充（4线通用，按 eastern_note 准确传统）
 * 读 articles-merged/{slug}.json → 通用概念 prompt（按 art.eastern_note 传统 framing）→ _placeholders-merged/{slug}.txt
 * BATCH=2。用法：node fill-ai-batch-merged.js [--slug=xxx]
 */
...
function buildPrompt(art) {
  ...
  return `You are an expert SEO content writer for Earthward (healing crystal jewelry). Write premium, evidence-grounded, CULTURALLY RESPECTFUL English content.

ARTICLE: ${art.title}
SUBJECT: ${art.title.replace(/:.*/, '')}
PRIMARY KEYWORD: ${art.rank_math_focus_keyword}

FACTUAL DATA (symbolic/reflective — translate to natural English, do NOT fabricate):
- Meaning: ${art.definition}
- How to engage: ${(calc.steps || []).join(' | ')}
- Caveat: ${art.caveat || 'none'}

CRYSTALS (THREE-ELEMENT each: science + tradition + mindfulness; MODERN associations):
${crystalBlock}

TRADITION (use EXACTLY, do NOT mix): ${art.eastern_note || 'cross-cultural symbolism'}

CRITICAL RULES:
1. CULTURAL RESPECT: use the tradition specified above. Frame as "in {tradition}...", "{tradition} associates...". Do NOT appropriate, do NOT claim exclusive truth.
2. NON-DETERMINISTIC: symbolic/reflective framework. NEVER "guarantees", "determines", "will definitely", "cures". Use: symbolizes / invites reflection / traditionally associated / many find.
3. CAVEAT: if art.caveat notes audience mismatch/intent confusion/overlap, frame content to acknowledge the specific nuance (e.g. for Vedic "modern crystal substitute for the traditional gem"; for HD "modern crystal association, not classical HD").
...
5. THREE PERSPECTIVES woven naturally: symbolism + crystal mineralogy + mindfulness/reflection.
