...
const QUEUE_PATH = path.join(IMAGES_DIR, 'dream-image-queue-repaired-full-1289.jsonl');
const RESULTS_PATH = path.join(IMAGES_DIR, 'dream-image-generation-results-repaired-full-1289.jsonl');
const SUMMARY_PATH = path.join(IMAGES_DIR, 'dream-image-generation-summary-repaired-full-1289.json');
...
function completedSlugs() {
  const done = new Set();
  for (const file of [path.join(IMAGES_DIR, 'dream-image-generation-results.jsonl'), RESULTS_PATH]) {
...
function outPathFor(item) {
  return path.join(OUTPUT_ROOT, item.slug, item.image.filename || `${item.slug}-hero.webp`);
}
...
async function generateOne(item) {
  ...
  const prompt = [
    item.image.hero_prompt,
    'Premium editorial dream image, calm and reflective, no text, no watermark, no logo.',
    Array.isArray(item.image.negative_prompt) ? `Avoid: ${item.image.negative_prompt.join(', ')}.` : '',
  ].filter(Boolean).join(' ');
  try {
...
async function main() {
  ...
  let queue = readJsonl(QUEUE_PATH).filter((item) => !done.has(item.slug)).slice(0, LIMIT);
