...
const idx = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'articles-planet-in-sign-index.json'), 'utf8'));
...
function buildSchema(a) {
  ...
  schemas.push({ "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://" + SITE + "/" }, { "@type": "ListItem", "position": 2, "name": "Astrology", "item": "https://" + SITE + "/astrology/" }, { "@type": "ListItem", "position": 3, "name": a.title.replace(/:.*/, ''), "item": url }] });
  return schemas.map(s => JSON.stringify(s)).join('\n');
}

const CAT_ID = ensureCategory('astrology', 'Astrology');
console.log('分类 numerology → ' + CAT_ID + '\n');
let ok = 0, fail = 0, skip = 0;
for (const art of list) {
  if (!slugArg && slugExists(art.slug)) { console.log('⏭ ' + art.slug + ' (已存在)'); skip++; continue; }
  try {
    const a = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'articles-planet-in-sign', art.slug + '.json'), 'utf8'));
