/**
 * Astrology 概念+Houses AI 批量填充（概念解释，非科学框架）
 * 读 articles-concepts/{slug}.json → astrology 概念 prompt（象征/反思+非科学+七曜平行）→ _placeholders-concepts/{slug}.txt
 * BATCH=2。用法：node fill-ai-batch-concepts.js [--slug=xxx]
 */
...
const ART_DIR = path.join(DIR, 'articles-concepts');
const PH_DIR = path.join(DIR, '_placeholders-concepts');
...
function buildPrompt(art) {
  ...
  return `You are an expert astrologer SEO writer for Earthward (healing crystal jewelry). Write premium, nuanced, NON-deterministic English about an astrology concept or house.

ARTICLE: ${art.title}
SUBJECT: ${art.title.replace(/:.*/, '')}${art.life_area ? ' (' + art.life_area + ')' : ''}
PRIMARY KEYWORD: ${art.rank_math_focus_keyword}

CONCEPT DATA (Western astrology — public classical):
- Definition: ${art.definition}
- Explanation points: ${(calc.steps || []).join(' | ')}

CRYSTALS (THREE-ELEMENT each; MODERN astrological associations):
${crystalBlock}

CRITICAL RULES:
1. NON-DETERMINISTIC + NOT SCIENCE: astrology is symbolic/archetypal language for self-reflection. NEVER "science proves", "you will/are destined", "determines", "guarantees". Use: symbolizes / invites reflection on / traditionally associated with / offers a framework for / many find meaning in.
2. BALANCE: every concept/house has light + shadow; present both.
3. EASTERN: parallel reference — 七曜 (Seven Luminaries: Sun/Moon + 5 planets → 火水木金土曜) as parallel frame, NOT historical equivalence.
...
5. THREE PERSPECTIVES woven naturally: astrological symbolism + crystal mineralogy + mindfulness/self-reflection.
6. For HOUSE pages: explain what life area the house governs + how planets/transits there manifest symbolically.
