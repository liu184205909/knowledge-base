...
### 三层标配（每篇 dream 内容必含）
> **科学谦逊桥（前置，非独立第四层）**：From Sleep Science 段讲梦的机制与边界（为什么梦生动/重复/和情绪相关），用科学谦逊句式（"research hasn't confirmed 某梦象 = 某固定含义"）为下面三层留合法空间，**不做梦象定论**。吸收自 sleepfoundation 的结构手法，**不冒用其 MD / peer-review / 机构背书**；religious lens 页此段只做边界铺垫，不压信仰/文化解读。

1. **心理学解读**：荣格原型 + 弗洛伊德视角（对标 dreamdictionary/dreammoods 强项）
2. **灵性/宗教解读**：Islamic（⭐ 头部薄覆盖 + 系统承接缺口）/ Biblical / Spiritual（三 lens，偏 Islamic）
3. **水晶对应推荐**：每篇 1-3 颗水晶，定位 **journaling ritual / reflection anchor**（梦境日记与反思锚点，**不做疗效承诺**）（→ /product-category/{stone}-crystals/ Shop 三级降级验证 200）
...
| 模块 | H2/H3 | 数据层字段 |
|---|---|---|
| **Key Takeaways** ⭐ | H2: Key Takeaways（3-4 条速览，含 **1 条 sleep science + 1 条 crystal reflection** takeaway）| `{key_takeaways}` |
...
| **From Sleep Science** 🧪 | H2: From Sleep Science（科学谦逊桥：梦的机制与边界，**不做梦象定论**）| `{sleep_science}` |
| **心理学解读** | H2: Psychological Reflection — The Psychology Behind {Symbol} Dreams | `{psychology_meaning}` |
| ↳ 荣格原型 | H3: Jungian Perspective | `{jung_archetype}` |
| ↳ 弗洛伊德 | H3: Freudian Interpretation | `{freud_angle}` |
| **灵性/宗教解读** | H2: Spiritual Symbolism & Religious Meanings | |
...
| **水晶承接** | H2: Crystals for {Symbol} Dream Reflection | `{crystal_recommendations}` |
| ↳ 推荐 1-3 颗 | H3: {Crystal Name} — {Reason}（journaling ritual / reflection anchor，**不做疗效承诺**）| |
...
## 套 4：Crystals×Dreams 水晶×梦交叉（变现层）
