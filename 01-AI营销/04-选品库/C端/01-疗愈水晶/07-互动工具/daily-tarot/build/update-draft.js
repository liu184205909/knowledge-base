// Update existing daily-tarot draft page (PUT /wp-json/wp/v2/pages/50319)
// 守 PRD 红线：status=draft，不 publish
const fs = require('fs');
...
const POST_ID = 50319;
const HTML = fs.readFileSync(path.resolve(__dirname, 'daily-tarot.html'), 'utf8');
const content = '<!-- wp:html -->\n' + HTML + '\n<!-- /wp:html -->';

const body = JSON.stringify({ content: content, status: 'draft' });
