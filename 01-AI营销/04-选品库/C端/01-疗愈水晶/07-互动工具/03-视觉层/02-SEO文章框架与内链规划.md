...
> **创建**：2026-06-25｜**更新**：2026-06-25（工具拆分后仅管 Crystal Checker 43180；水晶×水晶结果 CTA 跳 169 篇水晶配对文章 `/{a}-and-{b}/`）
...
|------|------|-----|------|------------|
| **Crystal Checker** | 43180 | `/tools/crystal-compatibility-checker/` | 两水晶能不能一起戴 | **`/{a}-and-{b}/`（169 篇水晶配对）** |
...
| 结果区 "Read Full Combination Guide →" CTA | **`/{stoneA}-and-{stoneB}/`** | 跳水晶配对文章（**仅 169 篇精选有文章时显示**） |
| 其余 266 组（无文章） | **不显示 CTA** | 结果页自给自足（评分+Why+Best For+How to Use+Shop） |
...
> **跳转桥接逻辑（水晶×水晶 → 水晶配对文章）**：结果直接对应 `/{stoneA}-and-{stoneB}/`（slug = 两石 slug 按字母序 `-and-` 拼接，与 selected-articles.json 的 slug 一致）。169 个 slug 内嵌工具 JS 的 `HAS_ARTICLE` 清单；命中则显示 CTA，不命中则隐藏。**不再做"水晶 Zodiac 交集→星座配对"的旧桥接**（那是拆分前的逻辑，现归 Zodiac Checker）。

## 四、水晶配对文章清单（169 篇，Crystal Checker CTA 目标）

`selected-articles.json` 的 169 个 slug（T1_pairing 65 + T2_highscore 91 + T3_conflict 13），全部内嵌进 Crystal Checker 页面 JS 的 `HAS_ARTICLE` 数组。生产完成后 CTA 全量生效；当前可先内嵌清单（数据已就绪），文章陆续上线后自动有 CTA。
...
- [ ] 169 篇水晶配对文章产出（Crystal Checker CTA 目标，见 [模板-水晶配对文章框架](../../03-内容策略/内容Brief/模板-水晶配对文章框架.md)）
- [ ] Zodiac Checker（43246）独立 SEO 框架（链 78 星座配对，另规划）
- [ ] Crystal Checker 页 JS 内嵌 169 个 HAS_ARTICLE slug（数据已就绪）
