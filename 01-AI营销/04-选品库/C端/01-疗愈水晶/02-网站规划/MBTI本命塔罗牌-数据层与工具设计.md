...
> **完成状态（2026-07-04 更新）**：A 数据层 ✅ + B 工具开发 ✅ + C 工具上线 ✅ 已全部完成。
> - 工具已上线 `/tools/mbti-tarot/`（WordPress page 51694，draft 状态待 publish）。
> - 数据层 `_shared/mbti-tarot-knowledge.json`（16 型完整字段）+ `mbti-tarot/build/mbti-quiz-4opt.json`（28 题测验）。
> - 工具形态：4 步下拉（路径 A）+ 28 题测验（替代原"选填生日"路径 B）双模式，28 题验证 A 级 + 已落实 P1/P2 改进（顺序偏差缓解 + Q27 区分）。
> - 联动：snippet 19（Keep exploring 18 工具联动）+ mega menu 已加。
> - 待做：D/E 文章矩阵（16 hub + ~32 交叉，暂不做见 §4.2）。
>
> 任务前置：竞品研究见 `01-竞品分析/MBTI本命塔罗牌-竞品研究.md`。
> 工具定位（2E-页面工具规划 §130）：T-MBTI，`/tools/mbti-tarot/`，独立工具，输入 MBTI 型 → 推荐塔罗本命大牌 + 个性化水晶。
> 本文档 = 设计稿（schema / 工具形态 / 文章规划）。
...
| 阶段 | 任务 | 数量 | 估时（并行）| 依赖 |
|---|---|---|---|---|
| **A. 数据层** ✅ 已完成（2026-07-04）| mbti-tarot-knowledge.json（16 型完整字段）+ mbti-quiz-4opt.json（28 题）| 16 型 × 8 字段 + 28 题 | **1-1.5 天** | 本文 §2.5 总表 user 审 |
| **B. 工具开发** ✅ 已完成（2026-07-04）| generate.js + HTML + SEO accordion + Code Snippet 部署 | 1 工具 | **1.5-2 天**（参照 daily-tarot 模式）| A 完成 |
| **C. 工具上线** ✅ 已完成（2026-07-04）| TKD + mega menu + 18 工具联动 CTA + sitemap | 1（page 51694，draft 待 publish）| **0.5 天** | B 完成 |
| **D. 文章矩阵第一阶段** ⬜ 暂不做 | 16 型 hub 文章 | 16 篇 | **2-3 天**（07 工作流并行）| A 完成（B/C 不阻塞）|
| **E. 文章矩阵第二阶段** ⬜ 暂不做 | 16×本命牌交叉 | ~32 篇 | **3-5 天**（并行）| D 完成 |
| **总计** | | ~49 篇 + 1 工具 + 1 json | **~8-11 天**（串行）/ **~5-7 天**（充分并行）| |

### 5.2 下一步顺序（推荐执行序）

```
1. ✅ User 审本文 §2.5 映射总表（16 型主牌/次牌）— 已确认
   ↓（确认后）
2. ✅ 生产数据层 mbti-tarot-knowledge.json（16 型完整字段 + 28 题测验）— 已完成 2026-07-04
   ↓（并行启动）
3a. ✅ 工具开发（generate.js / HTML / SEO / 部署）— 已完成 2026-07-04
3b. ⬜ 文章矩阵第一阶段（16 hub，不依赖工具上线）— 暂不做
   ↓
4. ✅ 工具上线（page 51694 draft，TKD + mega menu + 18 工具联动 CTA 已加）— 已完成 2026-07-04，待 publish
   ↓
5. ⬜ 文章矩阵第二阶段（~32 交叉文章）— 暂不做
   ↓
6. ⬜ 验收 + GSC 提交 + 内链融入 mega menu（文章矩阵启动后做）
```

### 5.3 待 user 决策点（均已决策，2026-07-04）

1. ✅ **映射总表 §2.5**：已确认（16 型主牌/次牌按设计稿执行）。
2. ✅ **双路径输入 §3.1**：已决策为双路径——路径 A（4 步下拉选 MBTI 型）+ 路径 B 改为 **28 题 MBTI 测验**（替代原"选填生日"），让未知 MBTI 型的用户也能用工具。28 题验证 A 级 + 已落实 P1/P2 改进（顺序偏差缓解 + Q27 区分）。
3. ✅ **文章规模 §4.2**：已决策——**文章矩阵暂不做**（D/E 阶段搁置），先把工具上线跑数据，文章矩阵视工具流量再决定。
4. ✅ **MBTI 商标 disclaimer**：已决策加——工具页 + 未来文章统一加免责声明（self-reflection framework，not affiliated with The Myers-Briggs Company）。
...
**本文档为设计稿**。A/B/C 阶段（数据层 + 工具 + 上线）已完成（2026-07-04，工具 `/tools/mbti-tarot/` page 51694）。下一步 = 视工具流量决定是否启动 D/E 文章矩阵。
