...
foreach ($needle in @('data-t17-ui', 'data-ring', 'data-grid', 'data-dialog="wrist"', 'data-dialog="finish"', 'data-stage-actions', 'data-action="background"', 'data-action="assemble"')) {
  if ($fragment -notmatch [regex]::Escape($needle)) { throw "Fragment is missing $needle" }
}
if ($fragment -match 'ew-t17-ui__tray-calibration') { throw 'Tray branding must not be rendered in the design canvas.' }
if ($script -notmatch [regex]::Escape('brand.remove()')) { throw 'UI script must remove any legacy tray branding from injected markup.' }
