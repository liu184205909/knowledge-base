...
### 数据层质量阀门（tarot-knowledge.json 验收，全塔罗普适见 [1F §0A.4](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)）⭐
...
- [ ] 22 牌 archetype 主体象征词组重叠 < 30%（[1F §0A.4](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)）
- [ ] 每牌 M3 reversed_keywords 写该牌独有 shadow（非 22 牌通用 resistance/blockage）
- [ ] **Health/Finances 相关牌**（Empress/Tower/Death/Devil/Justice 等）的 upright/reversed 意象不触 [1F §0A.1](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) 合规红线（config 层验收，不带到正文）
...
> 牌义页守 [1F §0A 全塔罗普适规则](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)（合规边界 / 禁用表达库 / 质检语义去重 / 数据层质量阀门）。本节只列**牌义页专属合规重点**，普适黑名单/阈值/验收清单见 1F §0A 对应小节。

**Health / Finances 合规（牌义页专属重点）**：
牌义页是单牌普适深度页，**涉及健康/财运 archetype 的牌**（如 The Empress=丰育/The Tower=身体震荡预警/Death=转化/The Devil=执念/Justice=平衡）在 M2/M3 落到健康或财运维度时，必须守 [1F §0A.1 Health/Finances 黑名单](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)：
...
- Health/Finances 主题牌的 FAQ（如 "Is The Tower a warning about my health?"）答案过 [1F §0A.1](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) 合规门，答 reflective not predictive
- 牌义页**不强制加 Health/Finances 收尾免责**（牌义页是普适页非场景页）；但若该牌 FAQ 明确谈健康/财运，在该 FAQ 答案内嵌免责口径

**禁用表达库**：全塔罗 6 类禁+替代表见 [1F §0A.2](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)。牌义页高发区：M4 水晶段（水晶万能句/医疗承诺/财务承诺）、M3 逆位段（恐吓逆位）、Intro 钩子（泛泛开头）。
...
**关卡 4 — 语义去重 + 结构指纹（全塔罗普适升级 ⭐，见 [1F §0A.3](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)）**：
