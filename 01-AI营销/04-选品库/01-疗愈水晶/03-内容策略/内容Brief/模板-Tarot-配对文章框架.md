...
> **竞品依据**：[1F-塔罗内容写法研究](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) §2.3 —— 竞品**集体空白**（Biddy/Labyrinthos 无系统配对文章，Biddy 付费 ebook 有 combinations 章节但非独立页；The Crystal Council 等合集页偶有"two cards together"一段）。这是**蓝海交叉长尾**，但也意味着**无强对标，自定高标准**（1500-2000 词/篇）。
...
**篇幅**：按张力分级（§5.1），下限不低于 lite 档 900 词（[1F §4.3](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) 下限 1200 仅适用于 standard/benchmark）
...
| 组件 | 执行依据 | 配对专属差异 |
|---|---|---|
| 合规/去AI化 | 牌义页 §16 + [1F §0A](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) | 同（禁预测/确定论/凶兆；化学式大写；句式轮换）+ **M3 禁三段拼接、M6 禁水晶并列句**；**配对涉及健康/财运组合时守 [1F §0A.1](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) Health/Finances 黑名单**（如 Death×The Sun 谈疗愈转化禁 cure、The Emperor×The Fool 谈投资冒险禁 invest now/guaranteed gain）；**禁用表达库 6 类见 [1F §0A.2](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)**（配对页高发区：M3 泛泛开头/万能搭配句、M6 水晶万能句）|
...
> 配对页特殊风险：两牌组合可触发 Health（疗愈/健康）或 Finances（财运/投资）语境，**组合后承诺风险高于单牌**（单牌 Death 谈"结束"，Death×Sun 谈"疗愈转化"易滑向 cure）。本节列配对专属 Health/Finances 组合红线（基于 [1F §0A.1](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) 黑名单 + 场景 v2.2 建议 10）。
...
- grep [1F §0A.1 Health/Finances 黑名单](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)（cure/diagnose/heals/invest now/guaranteed gain/this stock 等）+ [1F §0A.2 禁用表达库](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) 6 类
...
**关卡 5 — 语义去重 + 结构指纹（全塔罗普适升级 ⭐⭐⭐，配对页最易雷同，本框架核心存在意义，见 [1F §0A.3](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)）**：
...
### 9.1 pair-knowledge.json 数据层质量阀门（全塔罗普适见 [1F §0A.4](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)，本节配对专属 ⭐）
...
- [ ] **Health/Finances 相关对**（如 Death×Sun 谈疗愈、Emperor×Fool 谈投资、Devil×Tower 谈执念破局）的 story_seed/position_readings 不触 [1F §0A.1](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md) 合规红线 + **§7.1 配对专属 Health/Finances 红线**（config 层验收，不带到正文）
...
**抽审比例**：高权重对（高频/标杆关系类型）全审；一般对抽 30%；低频对抽 20%（对齐 [1F §0A.4 ③](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)）。
...
| [每日运势工具](../../07-互动工具/daily-tarot/PRD.md) | 日期×牌×水晶/星象（工具页，非文章）| 完全不同维度（日期驱动 vs 组合驱动）；原 365 篇文章计划已作废，改为 `/tools/daily-tarot/` 单工具页 |

**核心原则**（[1F §0 红线 1](../../01-竞品分析/1F-内容策略分析/塔罗内容写法研究.md)）：每篇锚定**独特组合数据**（配对页 = 牌×牌组合），正文落到该组合独有象征，禁止通用句跨篇复用。
