/**
 * 上传 Gift 文章（43 篇）到 WP(draft)
 * 根级 slug（/crystal-anniversary-gifts/）、hero only、schema(Article+FAQPage+ItemList+Breadcrumb)
 * category: crystal-gifts（新建）；rank_math: updateMeta；gift content 无 JS → wp:html 块直接放（wp_kses 兼容）
 * 用法：node upload.js [--slug=xxx | --offset=0 --limit=10]   需 socks5 + disableSandbox
 */
...
const CAT_ID = ensureCategory('crystal-gifts', 'Crystal Gifts');
console.log('分类 crystal-gifts → ' + CAT_ID + '\n');
