 ...
 * 读 zodiac-tarot-hubs-knowledge.json → 主词页模板（Quick Answer/What Is/Explained+HowTo/Key Facts/Crystals内链9交叉页/Eastern/FAQ/Related）
 * 无 by Number 段（meditation 无数字概念）；crystals 内链 zodiac-signs hub + 9 交叉页
 * 用法：node generate-mainpages.js
 */
const fs = require('fs'), path = require('path');
const DIR = path.resolve(__dirname, '..');
const KNOW = require('../../../07-互动工具/_shared/zodiac-tarot-hubs-knowledge.json');
...
for (const mp of all) {
  ...
  const crystalsHtml = (crystals || []).length ? `<h2>Crystals to Deepen Your Practice</h2>\n<p>{{AI_CRYSTALS_INTRO}}</p>\n<ul>\n${crystals.map(c => `<li><a href="/${c}-meaning/">${stoneName(c)}</a> — {{AI_CRYSTAL_${c.toUpperCase().replace(/-/g, '_')}}}}</li>`).join('\n')}\n</ul>\n${crystals.map(shopCTA).join('\n')}\n<p><em>Explore <a href="/zodiac-signs/">Best Crystals for Meditation</a>, <a href="/crystals-for-focus-meditation/">focus</a>, <a href="/crystals-for-sleep-meditation/">sleep</a>, <a href="/grounding-meditation-with-crystals/">grounding</a> guides.</em></p>` : '';
  ...
  const related = `<h2>Related Meditation Guides</h2>\n<ul>\n${relatedHtml}\n<li><a href="/zodiac-signs/">Best Crystals for Meditation</a></li>\n</ul>`;
