...
const idx = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'articles-mainpages-index.json'), 'utf8'));
const list = slugArg ? idx.articles.filter(a => a.slug === slugArg) : (LIMIT ? idx.articles.slice(OFFSET, OFFSET + LIMIT) : idx.articles.slice(OFFSET));
...
function buildSchema(a) {
  ...
  schemas.push({ "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://" + SITE + "/" }, { "@type": "ListItem", "position": 2, "name": "Feng Shui", "item": "https://" + SITE + "/feng-shui/" }, { "@type": "ListItem", "position": 3, "name": a.title.replace(/:.*/, ''), "item": url }] });
  return schemas.map(s => JSON.stringify(s)).join('\n');
}

const CAT_ID = ensureCategory('feng-shui', 'Feng Shui');
console.log('分类 feng-shui → ' + CAT_ID + '\n');
let ok = 0, fail = 0, skip = 0;
for (const art of list) {
  if (!slugArg && slugExists(art.slug)) { console.log('⏭ ' + art.slug + ' (已存在)'); skip++; continue; }
  try {
    const a = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'articles-mainpages', art.slug + '.json'), 'utf8'));
