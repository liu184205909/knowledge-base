...
function buildSchema(a) {
  ...
  schemas.push({ "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://" + SITE + "/" }, { "@type": "ListItem", "position": 2, "name": "Runes", "item": "https://" + SITE + "/runes/" }, { "@type": "ListItem", "position": 3, "name": a.title.replace(/:.*/, ''), "item": url }] });
  return schemas.map(s => JSON.stringify(s)).join('\n');
}

const CAT_ID = ensureCategory('runes', 'Runes');
