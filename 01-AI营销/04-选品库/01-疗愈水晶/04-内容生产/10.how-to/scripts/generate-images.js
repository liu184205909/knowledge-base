/**
 * How-to 生图（hero + moonlight/selenite/smoke 分步 + jewelry 场景）
 * source_type=scene：真实水晶摄影，无文字
 * gpt-image-2 via moleapi，loadEnv 强制覆盖 shell 错误占位 key（shell-envkey-override-loadenv）
 * 需 NODE_PATH="$(npm root -g)" sharp（crystal-scripts-nodepath-sharp）
 * 已存在的跳过（支持补生）
 * 用法：
 *   NODE_PATH="$(npm root -g)" node generate-images.js --slug=how-to-cleanse-crystals
 *   NODE_PATH="$(npm root -g)" node generate-images.js --type=hero  # 只生某类型
 * 参考：9.angel-numbers/scripts/generate-images.js
 */
...
function loadEnv() {
  ...
  if (fs.existsSync(envPath)) {
    for (const line of fs.readFileSync(envPath, 'utf8').split(/\r?\n/)) {
      const t = line.trim();
      if (!t || t.startsWith('#')) continue;  // 跳过注释行（含被注释的占位 key）
      const m = t.match(/^([A-Z_]+)\s*=\s*(.+)$/);
      if (m) process.env[m[1]] = m[2].trim().replace(/^["']|["']$/g, '');  // 无条件覆盖
    }
  }
}
loadEnv();
...
function buildPrompt(alt, sourceType) {
  ...
  if (s.includes('scene')) {
    // 真实水晶摄影，无文字（How-to 所有图都走这个）
    return `${alt}. Natural crystal photography, real gemstones with visible texture and light refraction, soft natural window light, calm muted background, realistic, no text, no watermark, no human face. Editorial crystal content quality, peaceful mindful ritual mood.`;
  }
  // 默认（graphic 风格，允许文字）— How-to 不用，保留兼容
  return `${alt}. Elegant spiritual graphic design, soft warm lighting, balanced composition, premium editorial quality, no watermark.`;
}
...
async function processImg(art, key, img) {
  ...
  // hero 1536x864 (source 1536x1024 -> cover), 其他 1200x800 (source 1536x1024 -> cover)
