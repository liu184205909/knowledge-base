...
### 可模仿点
1. **7 维度搜索** —— 脉轮/情绪/星座/行星/数字/元素/推荐，行业最全搜索维度（依据：§6.1 + 1H 模仿5）。落地：至少 4 维度（功效/脉轮/星座/元素）
2. **500+ 水晶数据库** —— 行业最大水晶百科（依据：§6.1 + 1H 模仿5）。落地：390 篇 Crystal Meaning 已接近
3. **订阅盒独立入口** —— Crystal Subscription Box 独立页面（依据：§6.1）。落地：核心产品方向，直接对标
4. **Premium 会员** —— $2.99/周，折扣+返现（依据：§6.1）。落地：订阅制+会员权益参考
5. **在线 Chat 支持** —— 透明等待时间（依据：§6.1）。落地：客户服务标准

### 可超越点
1. **DR 低（43）** —— 域名权重不足（依据：§6.2）。落地：通过内容+外链提升 DR
2. **无博客内容** —— 未见 Blog 板块，SEO 依赖水晶数据库（依据：§6.2）。落地：1500+ 博客内容页
3. **设计缺乏温度** —— 偏功能化，品牌情感弱（依据：§6.2）。落地：品牌故事+情感化内容

### 差异化机会
1. **工具+内容+电商融合体** —— TCC 是工具型，Tiny Rituals 是内容型，我们做融合体（依据：§6.3）。落地：互动工具+1500 篇内容+实物电商
2. **Crystal Identifier App 远期目标** —— TCC 有拍照识别 App（依据：§6.1）。落地：远期 AI 水晶识别工具
