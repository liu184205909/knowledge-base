...
> **定位**：PRNS 链接权威模型（PageRank_NS）—— 专利 + API Leak + DOJ 三源交叉验证
> **适用场景**：做外链 / 内链策略、理解链接权威传递机制、判断负面 SEO 有效性时
> **边界与衔接**：不讲内容（→ 01）、不讲排名机制全貌（→ 02）
...
这个概念对 [14 文档](./13-AI搜索研究与Prompt执行库.md)中的 GEO 多平台分发策略也有指导意义：选择分发平台时，优先选择那些在你主题区域接近种子的平台。
...
[14 文档](./13-AI搜索研究与Prompt执行库.md)中的多平台分发策略（325% 提升数据），在选择分发平台时可以用"准种子界面"概念做筛选：优先选择在你主题区域接近种子的平台——从它们获得的链接在 PRNS 管道里路径更短。

**3. 聚类-代表文档机制影响 AI 引用选择。**

AI 引擎引用的是**段落**而非页面（见 [01 文档](../01-营销方法论基础/03-内容生产与质检/01-内容质量与可检索性标准.md)的核心认知）。如果 Google 的聚类-代表文档机制在 AI 检索中被复用，那么被选为"代表文档"的页面更可能被 AI 引用。PRNS 的距离模型可能影响代表文档的选择。

**4. 内外链统一的路径模型影响"可检索性"。**

[10 文档](../01-营销方法论基础/03-内容生产与质检/01-内容质量与可检索性标准.md)的"Retrievable > Rankable"框架强调段落级可检索性。PRNS 告诉我们：一个段落的宿主页面如果距离遥远（路径长），即使内容优质也难以被检索到。**缩短路径 = 提高可检索性。**

---

## 附录：外链质量实操特征（Zyppy 2026 专家共识 × 哥飞实操）

> 来源：Zyppy 131 位专家调查（2026，103 因素 × 13,665 数据点，7 点量表）+ 哥飞社群实操（社区讨论级）。本文正文管机制层（PRNS 距离/权威传递），本附录只收**选购与纪律层**——买什么链接、怎么分布、哪些是坑。

### 好外链特征清单

| # | 特征 | 要点 |
|---|------|------|
| 1 | 高可信域名 | Wikipedia / 政府 / 大学 / 知名公益机构 |
| 2 | 主题相关页面 | 相关性高=贵；预算有限时"大方向相关"也收——不相关≠没用，只是没那么有用 |
| 3+4 | 域名级 vs 页面级权威**分开计** | 高权站的自定义目录页 = 拿不到站内链接的**孤岛页**，页面级权威≈0——Adsy 上可买的"高权重站链接"多属此类，无效。判断链接先看**页面**本身有没有入链和流量，不是看根域名 |
| 5 | **有真实流量的页面**（最好有自然搜索流量） | Nitin Manchanda（Botpresso）："100 个真实用户点的链接胜过 1000 个没人点的" |
| 6+8 | 引荐**域名数** vs 引荐**页面数**分计 | 两条独立信号：来源页自身被多少域名引用 / 被多少页面引用。只靠内链传权的页面传不出多少权重 |
| 9 | 锚文本=目标关键词最优，**但画像比例是生命线** | Geoff Kenyon（Pomar）：精准锚文本超比例后反噬——关键在锚文本画像里所占**比例**而非绝对数。正解：核心词的多种表达分散到多个链接制造多样性 |
| 10 | Nofollow 有效 | 效果小但为正，有就要 |
| 11 | 低质链接 / PBN / 被掌握证据的买卖链接 = **负面** | 垃圾链接被评为全调查最大负面因素之一（与正文 SpamBrain 管道一致：负面 SEO 走垃圾链接分类器，不走 PRNS） |

### 外链 → GEO 跨界证据

Andrew Shotland（Local SEO Guide）：客户获路透社一条提及+链接，**次周 ChatGPT 可见度提升 4 倍**——外链不止传递 PRNS 权重，还直接改写 AI 引用可见性（与 03 文档"共识层：多独立来源互相佐证"互证：高权威第三方提及是 AI 采信的核心证据链）。

### 时机判据（哥飞社群两三年多次检验）

- **新词新站**：可免外链拿排名（竞争真空期，相关性+行为数据够用）
- **老词新站**：无外链**不可能有结果**；有外链也不够，还需 On-Page + 行为数据——老词的 SERP 已被 NavBoost 数据浸泡多年的 incumbents 占据
