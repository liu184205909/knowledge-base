...
### 2.5 数据库 SaaS 变体：筛选维度变页面（2026-09 案例）

> 来源：独立开发者笔记（社区讨论，2026-09），案例 Investor Hunt（11 万投资人）/ Journalist Hunt（29,089 URL）/ AngelMatch。适用前提：**信息公开但分散、用户会反复筛选、结果能直接用于工作**。

**飞轮**：数据资产 → 批量落地页 → 长尾搜索 → 免费预览 → 试用与导出 → 数据继续扩张

**四项设计**：

| 设计 | 要点 | 实例 |
|------|------|------|
| **冷启动** | 先人工整理 300–1000 条高质量数据验证价值（用户按什么筛选/哪些字段值得付费），**不要一开始抓 10 万条垃圾** | Investor Hunt 起步=人工 1000 条 |
| **页面生成** | 行业/地区/阶段/人物/机构等筛选维度 → 独立可收录页；"AI investors in New York"类组合词单个量小但意图极强 | Journalist Hunt 按公开 sitemap 统计 29,089 URL |
| **转化分层** | 免费层公开：姓名/机构/地区/行业等可预览数据（只显示标题+全要登录=Google 不收+用户不信）；付费层锁：完整联系方式/批量筛选/CSV 导出/批量触达 | 三案例一致 |
| **紧邻购买的工具** | 免费工具解决上一步问题，下一步自然需要数据库（新闻稿生成器→"发给谁"→记者库） | Journalist Hunt 配套 4 个生成器 |

**最小执行顺序**：窄人群（有明确付费目的）→ 300–1000 条人工数据 → 5–10 个真有用的筛选字段 → 20–50 个高意图目录页 → 每页真实可预览 → 锁批量/导出 → 紧邻购买工具 → 按收录/注册/付费扩页。

**证据纪律（该文方法论干净处）**：公开 robots.txt sitemap 只能证实"在做 pSEO"，**不能证明 SEO 贡献了多少流量**——没有 GSC/分析后台不虚构渠道占比。竞品 pSEO 结构侦察用 robots.txt sitemap 统计 URL 数与维度分类（工具：02-自动化工具库/12-sitemap定位器）。

**与 Mueller 警告的对照**（见 03-SEO与GEO/03 §AI购物旅程）：Mueller 批评的是"遍历组合+填充内容"的垃圾 pSEO；本变体成立的前提是**每页有真实可预览的数据**（页面回答一个真实存在的搜索问题）——同一机制的两种命运取决于数据真实性，不是页面数量。壁垒=数据准确+持续更新+每页独立意图+免费到付费的自然路径。
...
### 7.3 GEO 视角的内容类型权重修正（Profound 实证，2026-09-22 入库）

ChatGPT 2026-08 转向 Evaluating 后引用结构剧变，内容类型的引用率权重需要重排：

| 内容类型 | 引用率/变化 | 对 7.1-7.2 策略的含义 |
|---|---|---|
| **软件/产品对比页** | **30%**（Ramp 案例）vs 博客 2% | BOFU 对比页是引用之王——但注意 Listicles 泛列表 -38%，**赢的是深度结构化对比（参数表+FAQ），不是 top10 清单** |
| 产品页 FAQ 模块 | **+848%** 展示频次 | PDP/品类页 FAQ 是 GEO 最高 ROI 元素（配套：视频 +103%/评分 +36%/规格表 +23%） |
| Documentation 型内容 | +84% | 规格文档/材料说明/尺寸指南按"官方文档"标准写 |
| 商业页面 | +30% | 品类页/服务落地页直接受益 |
| 泛 listicle / Social / UGC | **-38% / -91% / -71%** | 纯清单文和社媒依赖内容在 ChatGPT 引用层贬值 |

**Ramp 案例全貌**（Blyskal 招牌案例）：2024-11 起点品类可见性 3.2%/第 19 名 → 不做 100 篇博客，只做 **2 个战略对比页** → 30 天可见性 3.2%→**22.2%**（7 倍）、第 8 名、300+ 引用超此前全部内容总和；对照：$50M 预算竞对 4.1%、1 万索引页竞对 5.7%、Google 第 1 竞对 6.2%。**页面的"引用权"可以压倒站点体量**。

**结构化 feed 趋势**（Blyskal 对协议层的下注）："MCP 将绝对成为标准，尤其在商务领域"；四引擎商务协议：ChatGPT=Product Feed+ACP+Stripe、Google=Merchant Center+AP2+A2A+PayPal、Perplexity=Index API+Shop Pay——**"非结构化抓取 → 结构化 feed"**，电商站的 GMC/Product Feed 配置从 SEO 可选项升级为 GEO 必选项。

### 7.4 Zack Franklin 的博客策略
